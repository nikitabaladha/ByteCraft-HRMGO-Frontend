  ...
  useEffect(() => {
    const fetchBusinessSetting = async () => {
      try {
        ...
        setLogoDark(`http://localhost:3001${logoDark}` || "not found");
        console.log("logoDark", logoDark)
        setLogoLight(`http://localhost:3001${logoLight}`);
        setFavicon(`http://localhost:3001${favicon}`);

        setImagePreview(`http://localhost:3001${logoDark}` || "");
        setCertificatePreview(`http://localhost:3001${logoLight}` || "");
        setResumePreview(`http://localhost:3001${favicon}` || "");
      } catch (error) {
