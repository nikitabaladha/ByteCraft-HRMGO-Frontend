...
const EmpWarningMain = () => {
  ...
  useEffect(() => {
    const fetchNewUserContent = async () => {
      try {
        const response = await getAPI("/get-employee-warning-content");
        const mappedData = {
          // mail_from_name: response.data.mailFromName || "",
          subject: response.data.subject || "Employee Warning",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await postAPI("/employee-warning-content", formData);
               ...
               <div className="card mb-0 h-100">
                 <div className="card-header card-body">
                   ...
                   <div className="row">
                     ...
                     <p className="col-6">
                       Employee:{" "}
                       <span className="text-primary">{`{employee_warning_name}`}</span>
                     </p>
                     <p className="col-6">
                       Subject:{" "}
                       <span className="text-primary">{`{warning_subject}`}</span>
                     </p>
                     <p className="col-6">
                       Description:{" "}
                       <span className="text-primary">{`{warning_description}`}</span>
                     </p>
                   </div>
                 </div>
               </div>
