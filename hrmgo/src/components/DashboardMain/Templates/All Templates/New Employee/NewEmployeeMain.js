...
const NewEmployeeMain = () => {
  ...
  useEffect(() => {
    const fetchNewUserContent = async () => {
      try {
        const response = await getAPI("/get-new-employee-content");
        const mappedData = {
          // mail_from_name: response.data.mailFromName || "",
          subject: response.data.subject || "New Employee",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await postAPI("/new-employee-content", formData);
                  ...
                  <div className="card mb-0 h-100">
                    <div className="card-header card-body">
                      ...
                      <div className="row">
                        ...
                        <p className="col-6">
                          App Url: <span className="text-primary">{`{app_url}`}</span>
                        </p>
                        <p className="col-6">
                          Employee Name: <span className="text-primary">{`{employee_name}`}</span>
                        </p>
                        <p className="col-6">
                          Password:{" "}
                        ...
                        <p className="col-6">
                          Employee Email:{" "}
                          <span className="text-primary">{`{employee_email}`}</span>
                        </p>
                        <p className="col-6">
                          Employee Branch:{" "}
                          <span className="text-primary">{`{employee_branch}`}</span>
                        </p>
                        <p className="col-6">
                          Employee Department:{" "}
                          <span className="text-primary">{`{employee_department}`}</span>
                        </p>
                        <p className="col-6">
