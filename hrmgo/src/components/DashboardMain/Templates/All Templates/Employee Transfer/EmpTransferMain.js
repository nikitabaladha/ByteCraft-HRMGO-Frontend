...
const EmpTransferMain = () => {
  ...
  useEffect(() => {
    const fetchNewUserContent = async () => {
      try {
        const response = await getAPI("/get-employee-transfer-content");
        const mappedData = {
          mail_from_name: response.data.mailFromName || "",
          subject: response.data.subject || "Employee Transfer",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await postAPI("/employee-transfer-content", formData);
              ...
              <div className="card mb-0 h-100">
                <div className="card-header card-body">
                  ...
                  <div className="row">
                    ...
                    <p className="col-6">
                      Employee Name:{" "}
                      <span className="text-primary">{`{transfer_name}`}</span>
                    </p>
                     <p className="col-6">
                            Transfer Date:{" "}
                            <span className="text-primary">{`{transfer_date}`}</span>
                          </p>
                          <p className="col-6">
                            transfer_department:{" "}
                            <span className="text-primary">{`{transfer_department}`}</span>
                          </p>
                          <p className="col-6">
                            Transfer Branch:{" "}
...
export default EmpTransferMain;
