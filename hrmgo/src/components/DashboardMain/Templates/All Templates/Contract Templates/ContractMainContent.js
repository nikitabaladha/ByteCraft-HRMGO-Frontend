...
const ContractMainContent = () => {
  ...
  useEffect(() => {
    const fetchNewUserContent = async () => {
      try {
        const response = await getAPI("/get-contract-content");
        const mappedData = {
          // mail_from_name: response.data.mailFromName || "",
          subject: response.data.subject || "Contract",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await postAPI("/contract-content", formData);
               ...
               <div className="card mb-0 h-100">
                 <div className="card-header card-body">
                   ...
                   <div className="row">
                     ...
                     <p className="col-6">
                       Contract Subject:{" "}
                       <span className="text-primary">{`{contract_subject}`}</span>
                     </p>
                     <p className="col-6">
                       Contract Employee:{" "}
                       <span className="text-primary">{`{contract_employee}`}</span>
                     </p>
                     
                     <p className="col-6">
                       Contract Start Date:{" "}
                       <span className="text-primary">{`{contract_start_date}`}</span>
                     </p>
                    <p className="col-6">
                       Contract End Date:{" "}
                       <span className="text-primary">{`{contract_end_date}`}</span>
                     </p>
                   </div>
                 </div>
               </div>
