...
const LeaveStatusMain = () => {
  ...
  useEffect(() => {
    const fetchNewUserContent = async () => {
      try {
        const response = await getAPI("/get-leave-status-content");
        const mappedData = {
          // mail_from_name: response.data.mailFromName || "",
          subject: response.data.subject || "Leave Status",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await postAPI("/leave-status-content", formData);
               ...
               <div className="card mb-0 h-100">
                 <div className="card-header card-body">
                   ...
                   <div className="row">
                     ...
                     <p className="col-6">
                       Leave Email:{" "}
                       <span className="text-primary">{`{leave_email}`}</span>
                     </p>
                     <p className="col-6">
                       Leave Status:{" "}
                       <span className="text-primary">{`{leave_status}`}</span>
                     </p>
                     <p className="col-6">
                       Employee Name:{" "}
                       <span className="text-primary">{`{leave_status_name}`}</span>
                     </p>
                     <p className="col-6">
                       Leave Reason:{" "}
                       <span className="text-primary">{`{leave_reason}`}</span>
                     </p>
                     <p className="col-6">
                       Leave Start Date:{" "}
                       <span className="text-primary">{`{leave_start_date}`}</span>
                     </p>
                     <p className="col-6">
                       Leave End Date:{" "}
                       <span className="text-primary">{`{leave_end_date}`}</span>
                     </p>
                     <p className="col-6">
                       Total Leave Days:{" "}
                       <span className="text-primary">{`{total_leave_days}`}</span>
                     </p>
                   </div>
                 </div>
               </div>
