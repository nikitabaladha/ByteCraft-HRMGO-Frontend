...
const NewTicketMain = () => {
  ...
  useEffect(() => {
    const fetchNewUserContent = async () => {
      try {
        const response = await getAPI("/get-new-ticket-content");
        const mappedData = {
          // mail_from_name: response.data.mailFromName || "",
          subject: response.data.subject || "New Ticket",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await postAPI("/new-ticket-content", formData);
                        ...
                        <div className="card mb-0 h-100">
                          <div className="card-header card-body">
                            ...
                            <div className="row">
                              ...
                              <p className="col-6">
                                Ticket Title: <span className="text-primary">{`{ticket_title}`}</span>
                              </p>
                              <p className="col-6">
                                Ticket Employee Name:{" "}
                                <span className="text-primary">{`{ticket_name}`}</span>
                              </p>
                              <p className="col-6">
                                Ticket Code:{" "}
                                <span className="text-primary">{`{ticket_code}`}</span>
                              </p>
                              <p className="col-6">
                                Ticket Description:{" "}
                                <span className="text-primary">{`{ticket_description}`}</span>
                              </p>
                            </div>
                          </div>
                        </div>
