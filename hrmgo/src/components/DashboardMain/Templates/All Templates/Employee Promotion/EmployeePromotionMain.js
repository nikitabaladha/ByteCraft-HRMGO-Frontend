   ...
   const [formData, setFormData] = useState({
    ...
    useEffect(() => {
      const fetchNewUserContent = async () => {
        try {
          const response = await getAPI("/get-employee-promotion-content");
          const mappedData = {
            // mail_from_name: response.data.mailFromName || "",
            subject: response.data.subject || "Employee Promotion",
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        await postAPI("/employee-promotion-content", formData);
                 ...
                 <div className="card mb-0 h-100">
                   <div className="card-header card-body">
                     ...
                     <div className="row">
                       ...
                       <p className="col-6">
                         Employee Name:{" "}
                         <span className="text-primary">{`{employee_promotion_name}`}</span>
                       </p>
                       <p className="col-6">
                         Designation:{" "}
                         <span className="text-primary">{`{promotion_designation}`}</span>
                       </p>
                       <p className="col-6">
                         Promotion Title:{" "}
                         <span className="text-primary">{`{promotion_title}`}</span>
                       </p>
                        <p className="col-6">
                         Promotion Date:{" "}
                         <span className="text-primary">{`{promotion_date}`}</span>
                       </p>
                       {/*<p className="col-6">
                       ...
                       <form onSubmit={handleSubmit}>
                         <div className="form-group col-12">
                           ...
                           <input
                             ...
                             name="subject"
                             type="text"
                             defaultValue="Employee Promotion"
                             id="subject"
                           />
                         </div>
                         <div className="form-group col-12">
...
export default EmployeePromotionMain
