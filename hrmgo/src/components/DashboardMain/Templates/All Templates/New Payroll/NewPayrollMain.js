...
const NewPayrollMain = () => {
  ...
  useEffect(() => {
    const fetchNewUserContent = async () => {
      try {
        const response = await getAPI("/get-new-payroll-content");
        const mappedData = {
          // mail_from_name: response.data.mailFromName || "",
          subject: response.data.subject || "New Payroll",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await postAPI("/new-payroll-content", formData);
              ...
              <div className="card mb-0 h-100">
                <div className="card-header card-body">
                  ...
                  <div className="row">
                    ...
                    <p className="col-6">
                      Salary Month:{" "}
                      <span className="text-primary">{`{salary_month}`}</span>
                    </p>
