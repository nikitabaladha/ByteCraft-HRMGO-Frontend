...
const EmpTerminationMain = () => {
  ...
  useEffect(() => {
    const fetchNewUserContent = async () => {
      try {
        const response = await getAPI("/get-employee-termination-content");
        const mappedData = {
          // mail_from_name: response.data.mailFromName || "",
          subject: response.data.subject || "Employee Termination",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await postAPI("/employee-termination-content", formData);
              ...
              <div className="card mb-0 h-100">
                <div className="card-header card-body">
                  ...
                  <div className="row">
                    ...
                    <p className="col-6">
                      Employee Name:{" "}
                      <span className="text-primary">{`{employee_termination_name}`}</span>
                    </p>
                    <p className="col-6">
                      Notice Date:{" "}
                    ...
                    <p className="col-6">
                      Termination Date:{" "}
                      <span className="text-primary">{`{termination_date}`}</span>
                    </p>
                    <p className="col-6">
                      Termination Type:{" "}
                      <span className="text-primary">{`{termination_type}`}</span>
                    </p>
                  </div>
                </div>
              </div>
                    ...
                    <form onSubmit={handleSubmit}>
                      <div className="form-group col-12">
                        ...
                        <input
                          ...
                          name="subject"
                          type="text"
                          defaultValue="Employee Termination"
                          id="subject"
                        />
                      </div>
                      <div className="form-group col-12">
...
export default EmpTerminationMain;
