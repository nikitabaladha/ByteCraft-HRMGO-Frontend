...
function ConfirmationDialog({ onCancel, contracts }) {
  const handleDelete = async () => {
    try {
      const response = await deleteAPI(`/contract/${contracts.id}`, {}, true);

      if (!response.hasError) {
        onCancel();

        toast.success("Contract successfully deleted!");
      } else {
        console.error("Error deleting Contract:", response.message);
        toast.error(`Failed to delete Contract: ${response.message}`);
      }
    } catch (error) {
      console.error("Error while deleting Contract:", error);
      toast.error("An error occurred while deleting Contract.");
    }
  };
  useEffect(() => {
