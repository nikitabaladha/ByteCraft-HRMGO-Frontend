...
const InterviewScheduleEdit = ({schedule, onClose}) => {
  ...
  const [formValues, setFormValues] = useState({
    ...
    synchronize_type: schedule.synchronizeType || false,
  });
  ...
  useEffect(() => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await putAPI(`/update-interview-schedule/${schedule._id}`, formValues);
            toast(response.data.message || "Schedule updated successfully!");
            onClose(); 
          } catch (error) {
  ...
  return (
    <div class="modal fade show" id="commonModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-modal="true" style={{display: 'block'}}>
