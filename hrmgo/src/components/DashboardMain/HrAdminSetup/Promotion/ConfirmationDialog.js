...
function ConfirmationDialog({ onCancel, promotion }) {
  const handleDelete = async () => {
    try {
      const response = await deleteAPI(`/promotion/${promotion.id}`, {}, true);
      ...
      if (!response.hasError) {
        ...
        console.error("Error deleting Promotion:", response.message);
        toast.error(`Failed to delete Promotion: ${response.message}`);
      }
    } catch (error) {
