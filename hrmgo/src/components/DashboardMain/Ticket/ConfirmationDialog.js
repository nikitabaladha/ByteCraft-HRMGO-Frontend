...
function ConfirmationDialog({ onClose, ticketId, onDeleted, deleteType }) {
  const handleDelete = async () => {
    try {
      ...
      if (deleteType === "ticket") {
        endpoint = `/ticket-delete/${ticketId}`; // Updated endpoint for ticket
      ...
      if (!response.hasError) {
        ...
        if (typeof onDeleted === "function") {
          onDeleted(ticketId); // Call the onDeleted callback with ticketId
        }
