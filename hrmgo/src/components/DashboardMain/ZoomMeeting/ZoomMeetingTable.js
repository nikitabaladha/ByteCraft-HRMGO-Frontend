  ...
  const handleStartMeeting = async (meetingId) => {
    try {
      const response = await putAPI(
        `/update_meeting_status/${meetingId}`,
        { status: "Starting" },
        true
      );
  ...
  const updateMeetingStatus = async () => {
    ...
    setMeetings((prevMeetings) => {
      return prevMeetings.map((meeting) => {
        const meetingEnd = dayjs(meeting.start_date).add(
          meeting.duration,
          "minute"
        );
        ...
        if (meeting.status === "Starting" && now.isAfter(meetingEnd)) {
          ...
          putAPI(
            `/update_meeting_status/${meeting._id}`,
            { status: "Ended" },
            true
          ).catch((err) =>
            console.error("Error updating meeting status on server:", err)
          );
