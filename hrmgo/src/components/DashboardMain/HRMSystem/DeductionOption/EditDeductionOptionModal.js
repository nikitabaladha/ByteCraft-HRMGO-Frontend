...
const EditDeductionOptionModal = ({ closeModal, deductionOption }) => {
    ...
    useEffect(() => {
        if (deductionOption) {
            setName(deductionOption.deductionName);
        }
    }, [deductionOption]);
    ...
    const handleSubmit = async (e) => {
        ...
        if (!deductionOption?._id) {
            toast.error("Invalid deduction option details.");
            return;
        }
        ...
        try {
            const response = await putAPI(`/update-deduction-option/${deductionOption._id}`, deductionOptionData, true);
            ...
            toast.error("An error occurred while updating the deduction option.");
        }
    };
...
export default EditDeductionOptionModal;
