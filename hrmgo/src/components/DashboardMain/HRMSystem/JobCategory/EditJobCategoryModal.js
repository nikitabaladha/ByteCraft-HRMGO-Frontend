...
const EditJobCategoryModal = ({ closeModal, jobCategory }) => {
    ...
    useEffect(() => {
        if (jobCategory) {
            setName(jobCategory.jobCategory); 
        }
    }, [jobCategory]);
    ...
    const handleSubmit = async (e) => {
        ...
        if (!jobCategory?._id) {
            toast.error("Invalid job category details.");
            return;
        }
        ...
        try {
            const response = await putAPI(`/update-job-category/${jobCategory._id}`, jobCategoryData, true); 
            ...
            toast.error("An error occurred while updating the job category.");
        }
    };
            ...
            <div className="modal-dialog modal-undefined" role="document">
                <div className="modal-content">
                    <div className="modal-header">
                        <h5 className="modal-title" id="exampleModalLabel">Edit Job Category</h5> {/* Updated title */}
