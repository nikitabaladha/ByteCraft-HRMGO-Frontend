...
const CreateJobCategoryModal = ({ closeModal, fetchJobCategories }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!name) {
      toast.error("Job category name is required.");
      return;
    }
    ...
    try {
      const response = await postAPI("/create-job-category", jobCategoryData, true);
      ...
      if (!response.hasError) {
        ...
        toast.error(`Failed to create job category: ${response.message}`);
      }
    } catch (error) {
      toast.error("An error occurred while creating the job category.");
    }
  };
      ...
      <div className="modal-dialog modal-undefined" role="document">
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title" id="exampleModalLabel">Create New Job Category</h5>
