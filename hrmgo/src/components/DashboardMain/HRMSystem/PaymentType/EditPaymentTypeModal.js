...
const EditPaymentTypeModal = ({ closeModal, paymentType }) => {
    ...
    useEffect(() => {
        if (paymentType) {
            setName(paymentType.paymentName);
        }
    }, [paymentType]);
    ...
    const handleSubmit = async (e) => {
        ...
        if (!paymentType?._id) {
            toast.error("Invalid payment type details.");
            return;
        }
        ...
        try {
            const response = await putAPI(`/update-payment-type/${paymentType._id}`, paymentTypeData, true);
            ...
            <div className="modal-dialog modal-undefined" role="document">
                <div className="modal-content">
                    <div className="modal-header">
                        <h5 className="modal-title" id="exampleModalLabel">Edit Payment Type</h5>
