...
const CreateLoanModal = ({ onClose, employee,toggleUpdateNetSalaryModal }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await postAPI('/create_loan', loanData, true);
            ...
            <div className="modal-dialog modal-md modal-lg" role="document">
                <div className="modal-content">
                    <div className="modal-header">
                        <h5 className="modal-title" id="exampleModalLabel">Create Loan Deduction</h5>
                        <button type="button" onClick={onClose} className="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <form  onSubmit={handleSubmit} method="POST" action="https://demo.workdo.io/hrmgo/loan" acceptCharset="UTF-8" className="needs-validation" noValidate>
