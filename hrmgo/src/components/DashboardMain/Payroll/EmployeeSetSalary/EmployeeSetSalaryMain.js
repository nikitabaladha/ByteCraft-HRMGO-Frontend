...
const EmployeeSetSalary = () => {
  ...
  const [isOtherpaymentModalOpen, setIsOtherpaymentnModalOpen] =
    useState(false);
    ...
    if (selectedAllowance) {
      setAllowances((prevAllowances) =>
        prevAllowances.filter(
          (allowance) => allowance._id !== selectedAllowance._id
        )
      );
    } else if (selectedCommission) {
      setCommissions((prevCommissions) =>
        prevCommissions.filter(
          (commission) => commission._id !== selectedCommission._id
        )
      );
      ...
      setOtherPayments((prevOtherPayments) =>
        prevOtherPayments.filter(
          (payment) => payment._id !== selectedOtherPayment._id
        )
      );
    } else if (selectedTax) {
      setTaxes((prevTaxes) =>
      ...
      setOvertimes((prevOvertimes) =>
        prevOvertimes.filter(
          (overtime) => overtime._id !== selectedOvertime._id
        )
      );
    }
    setIsDeleteDialogOpen(false);
  ...
  useEffect(() => {
    const fetchEmployeeData = async () => {
      try {
        const nameResponse = await getAPI(
          `/getemployeeemployee/${employeeId}`,
          {},
          true
        );
    ...
    const fetchAllowanceData = async () => {
      try {
        const allowanceResponse = await getAPI(
          `/getallowancebyid/${employeeId}`,
          {},
          true
        );
    ...
    const fetchCommissionData = async () => {
      try {
        const commissionResponse = await getAPI(
          `/getcommissionbyid/${employeeId}`,
          {},
          true
        );
    ...
    const fetchLoanData = async () => {
      try {
        const loanResponse = await getAPI(
          `/getloanbyid/${employeeId}`,
          {},
          true
        );
    ...
    const fetchOtherPaymentData = async () => {
      try {
        const otherPaymentResponse = await getAPI(
          `/getotherpaymentbyid/${employeeId}`,
          {},
          true
        );
    ...
    const fetchOvertimeData = async () => {
      try {
        const overtimeResponse = await getAPI(
          `/getovertimebyid/${employeeId}`,
          {},
          true
        );
  ...
  const totalOtherPayment = otherPayments.reduce(
    (sum, payment) => sum + payment.amount,
    0
  );
  const totalOvertimeRate = overtimes.reduce(
    (sum, overtime) => sum + overtime.amount,
    0
  );

  const totalDeductions = totalLoan + totalTax + totalOtherPayment;
  const NetSalary =
    totalAllowance + totalCommission - totalDeductions + totalOvertimeRate;
  ...
  useEffect(() => {
    if (employee?.id) {
      const fetchSalaryData = async () => {
        try {
          const response = await getAPI(
            `/getemployeedatabyid/${employee.id}`,
            {},
            true
          );
    ...
    <div className="row">
      ...
      <div className="col-xl-6">
        <div className="card set-card">
          ...
          <div
            className="card-body table-border-style"
            style={{ overflow: "auto" }}
          >
                ...
                <tbody>
                  {allowances.map((allowance, index) => (
                    <tr key={index}>
                      ...
                      <td className="text-dark">{`₹${new Intl.NumberFormat(
                        "en-IN"
                      ).format(allowance.amount)}`}</td>
                ...
                <tbody>
                  {loans.map((loan, index) => (
                    <tr key={index}>
                      ...
                      <td className='text-dark'>{loan.loanOption}</td>
                ...
                <tbody>
                  {taxes.map((tax, index) => (
                    <tr key={index}>
                      ...
                      <td className='text-dark'>{tax.taxes}</td>
              ...
              <table className="table">
                ...
                <tbody>
                  {otherPayments.map((payment, index) => (
                    <tr key={index}>
                      <td className='text-dark'>{payment.employeeName}</td>
                      <td className='text-dark'>{payment.title}</td>
                      <td className='text-dark'>{payment.type}</td>
                      <td className='text-dark'>{formatOtherpayment(payment)}</td>
                      <td className='text-dark'>{`₹${new Intl.NumberFormat("en-IN").format(
                        payment.amount
                      )}`}</td>
